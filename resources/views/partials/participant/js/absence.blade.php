    ...
    <script>
        ...
        function openCheckInForm() {
            Swal.fire({
                ...
                preConfirm: (jamMasuk) => {
                    if (!jamMasuk) {
                        Swal.showValidationMessage('Jam masuk wajib diisi');
                        return false;
                    }

                    const formData = new FormData();
                    formData.append('date', '{{ now()->toDateString() }}');
                    formData.append('status', jamMasuk > '08:00' ? 'late' : 'present');
                    formData.append('check_in', jamMasuk);

                    return fetch(`{{ route('participant.storeParticipant') }}`, {
                        method: 'POST',
                        headers: {
                ...
                if (result.isConfirmed) {
                    Swal.fire('Berhasil!', 'Jam masuk tercatat.', 'success').then(() => location.reload());
                }
            });
        }
        ...
        function openCheckOutForm() {
            Swal.fire({
                title: 'Isi Jam Keluar',
                input: 'time',
                inputLabel: 'Jam Keluar',
                inputValue: '16:00',
                showCancelButton: true,
                confirmButtonText: 'Simpan',
                preConfirm: (jamKeluar) => {
                    if (!jamKeluar) {
                        Swal.showValidationMessage('Jam keluar wajib diisi');
                        return false;
                    }

                    return fetch(`{{ url('/peserta/absen-keluar') }}`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                        ...
                        const data = await res.json();
                        if (!res.ok) throw new Error(data.message || 'Gagal simpan jam keluar');
                        return data;
                    }).catch(err => {
                        Swal.showValidationMessage(err.message);
                    });
                ...
                if (result.isConfirmed) {
                    Swal.fire('Berhasil!', 'Jam keluar tersimpan.', 'success').then(() => location.reload());
                }
            });
        }
    </script>
